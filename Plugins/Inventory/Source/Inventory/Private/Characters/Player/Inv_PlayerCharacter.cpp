...
#include "Characters/Player/Inv_PlayerCharacter.h"
...
// Sets default values
AInv_PlayerCharacter::AInv_PlayerCharacter()
{
	PrimaryActorTick.bCanEverTick = true;

	//create and set interaction sphere
	InteractSphere = CreateDefaultSubobject<USphereComponent>(TEXT("Sphere"));
	InteractSphere->OnComponentBeginOverlap.AddDynamic(this, &AInv_PlayerCharacter::OnSphereBeginOverlap);
	InteractSphere->OnComponentEndOverlap.AddDynamic(this, &AInv_PlayerCharacter::OnSphereEndOverlap);
...
void AInv_PlayerCharacter::BeginPlay()
{
	Super::BeginPlay();
...
void AInv_PlayerCharacter::Tick(float DeltaTime)
{
...
void AInv_PlayerCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	Super::SetupPlayerInputComponent(PlayerInputComponent);

	if (UEnhancedInputComponent* PlayerInput = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		PlayerInput->BindAction(MovementAction, ETriggerEvent::Triggered, this, &AInv_PlayerCharacter::Move);
		PlayerInput->BindAction(LookAction, ETriggerEvent::Triggered, this, &AInv_PlayerCharacter::Look);
		PlayerInput->BindAction(JumpAction, ETriggerEvent::Started, this, &AInv_PlayerCharacter::Jump);
		PlayerInput->BindAction(InteractAction, ETriggerEvent::Started, this, &AInv_PlayerCharacter::Interact);
	}
	
}
void AInv_PlayerCharacter::Move(const FInputActionValue& Value)
     {
...
void AInv_PlayerCharacter::Look(const FInputActionValue& Value)
{
	const FVector2D LookAxisVector = Value.Get<FVector2D>();
	AddControllerPitchInput(LookAxisVector.Y);
	AddControllerYawInput(LookAxisVector.X);
}
void AInv_PlayerCharacter::Jump(const FInputActionValue& Value)
{
	if (Value.Get<bool>())
	{
		ACharacter::Jump();
...
void AInv_PlayerCharacter::OnSphereBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor,
...
void AInv_PlayerCharacter::OnSphereEndOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor,
...
void AInv_PlayerCharacter::Interact()
{
